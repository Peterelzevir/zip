...
createRepoScene.on('text', async (ctx) => {
  ...
  try {
    ...
    await ctx.telegram.editMessageText(
      ...
      statusMsg.message_id,
      undefined,
      `✅ Repository baru berhasil dibuat: ${repoFullName} (Private)`,
      Markup.inlineKeyboard([
        Markup.button.callback('🔄 Ubah ke Public', 'toggle_visibility'),
...
selectRepoScene.action(/select_repo:(.+):(.+)/, (ctx) => {
  ...
  ctx.deleteMessage();
  ctx.reply(
    `✅ Repository terpilih: ${repoFullName} (${isPrivate ? 'Private' : 'Public'})`,
    Markup.inlineKeyboard([
      Markup.button.callback(`🔄 Ubah ke ${isPrivate ? 'Public' : 'Private'}`, 'toggle_visibility'),
      Markup.button.callback('▶️ Lanjutkan Push Ke GitHub', 'process_zip')
    ])
  );
...
bot.action('about', (ctx) => {
  ...
  return ctx.reply(
    ...
    '🔐 Kemampuan mengubah visibilitas repository\n' +
...
bot.action('toggle_visibility', async (ctx) => {
  ...
  if (!ctx.session || !ctx.session.github || !ctx.session.github.repo) {
    return ctx.reply('❌ Data repository tidak ditemukan.');
  }
  
  const { token, repo, isPrivate } = ctx.session.github;
  const [owner, repo_name] = repo.split('/');
  
  const statusMsg = await ctx.reply(`🔄 Mengubah visibilitas repository menjadi ${isPrivate ? 'Public' : 'Private'}...`);
  ...
  try {
    ...
    await ctx.telegram.editMessageText(
      ...
      statusMsg.message_id,
      undefined,
      `✅ Visibilitas repository berhasil diubah menjadi ${!isPrivate ? 'Private' : 'Public'}!`,
      Markup.inlineKeyboard([
        Markup.button.callback(`🔄 Ubah ke ${!isPrivate ? 'Public' : 'Private'}`, 'toggle_visibility'),
        Markup.button.callback('▶️ Lanjutkan Push Ke GitHub', 'process_zip')
      ])
    ...
    await ctx.telegram.editMessageText(
      ...
      Markup.inlineKeyboard([
        Markup.button.callback('🔄 Coba Lagi', 'toggle_visibility'),
        Markup.button.callback('▶️ Lanjutkan Push Ke GitHub', 'process_zip')
      ])
    );
  }
...
// Add a specific upload_zip action
bot.action('upload_zip_action', (ctx) => {
  ctx.deleteMessage();
  ctx.reply(
    '📤 Silakan unggah file ZIP yang ingin diproses.',
    Markup.inlineKeyboard([
...
const ADMIN_IDS = [123456789]; // Ganti dengan ID Telegram admin
const tempDir = path.join(os.tmpdir(), 'telegram-zip-github-bot');
